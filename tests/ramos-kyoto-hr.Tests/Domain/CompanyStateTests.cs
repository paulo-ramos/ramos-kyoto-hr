...
public class CompanyStateTests
{
    ...
    {
        // Arrange
        var company = new Company(_effectiveDate, _razaoSocial, _cnpj);
        company.Disable(_effectiveDate);
        var updatedAtBefore = company.UpdatedAt;

        // Act
        var hasChanged = company.Enable(_effectiveDate.AddDays(1));

        // Assert
        hasChanged.Should().BeTrue("porque o estado mudou de inativo para ativo");
        company.IsActive.Should().BeTrue("porque a empresa foi ativada");
        company.IsStatusActive().Should().BeTrue();
        company.UpdatedAt.Should().BeAfter(updatedAtBefore!.Value, "porque houve mudança de estado");
    }
    ...
    public void Enable_WhenCompanyIsAlreadyActive_ShouldNotChange_AndReturnFalse()
    {
        ...
        // Assert
        hasChanged.Should().BeFalse("porque a empresa já estava ativa");
        company.IsActive.Should().BeTrue("porque continua ativa");
        company.UpdatedAt.Should().Be(updatedAtBefore, "porque não houve mudança de estado (idempotente)");
    ...
    public void Enable_CalledMultipleTimes_ShouldBeIdempotent()
    {
        ...
        var updatedAtAfterFirstCall = company.UpdatedAt;
        
        var secondCall = company.Enable(_effectiveDate.AddDays(2));
        var thirdCall = company.Enable(_effectiveDate.AddDays(3));

        // Assert
        firstCall.Should().BeTrue("porque a primeira chamada mudou o estado");
        secondCall.Should().BeFalse("porque já estava ativo");
        thirdCall.Should().BeFalse("porque já estava ativo");
        company.IsActive.Should().BeTrue();
        company.UpdatedAt.Should().Be(updatedAtAfterFirstCall, "porque as chamadas subsequentes não mudaram nada");
    }
    ...
    public void Disable_WhenCompanyIsActive_ShouldDeactivate_AndReturnTrue()
    {
        ...
        // Assert
        hasChanged.Should().BeTrue("porque o estado mudou de ativo para inativo");
    ...
    public void Disable_WhenCompanyIsAlreadyInactive_ShouldNotChange_AndReturnFalse()
    {
        ...
        // Assert
        hasChanged.Should().BeFalse("porque a empresa já estava inativa");
    ...
    [Fact]
    public void Disable_CalledMultipleTimes_ShouldBeIdempotent()
    {
        // Arrange
        var company = new Company(_effectiveDate, _razaoSocial, _cnpj);

        // Act
        var firstCall = company.Disable(_effectiveDate.AddDays(1));
        var updatedAtAfterFirstCall = company.UpdatedAt;
        
        var secondCall = company.Disable(_effectiveDate.AddDays(2));
        var thirdCall = company.Disable(_effectiveDate.AddDays(3));

        // Assert
        firstCall.Should().BeTrue("porque a primeira chamada mudou o estado");
        secondCall.Should().BeFalse("porque já estava inativo");
        thirdCall.Should().BeFalse("porque já estava inativo");
    ...
    public void StateTransition_FromActiveToInactiveToActive_ShouldWorkCorrectly()
    {
        ...
        disableResult.Should().BeTrue("porque mudou de estado");
        company.IsActive.Should().BeFalse();

        // Transição 2: INATIVO → ATIVO
        var enableResult = company.Enable(_effectiveDate.AddDays(2));
        enableResult.Should().BeTrue("porque mudou de estado");
        company.IsActive.Should().BeTrue();
    }
    ...
    public void UpdatedAt_ShouldOnlyChangeWhenStateActuallyChanges()
    {
        ...
        company.UpdatedAt.Should().Be(initialUpdatedAt, "porque não houve mudança real de estado");
        ...
        company.UpdatedAt.Should().BeAfter(initialUpdatedAt!.Value, "porque houve mudança real de estado");
    }
